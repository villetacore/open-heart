...
> Быстрые ссылки: [индекс проекта](INDEX.md) · [архитектура](ARCHITECTURE.md) ·
> [редактор игры](EDITOR.md) · [форматы данных](DATA_FORMATS.md)
...
### Редактор игры (панель «OpenHeart» в Godot)

Открой проект в редакторе — сверху, рядом с 2D/3D/Script, есть вкладка **OpenHeart**.
Это редактор контента: враги, предметы, оружие, перки, квесты, NPC, блоки карты.
- Слева категория и записи, справа форма полей. «💾 Сохранить пресет» пишет JSON на диск.
- «Создать копию» делает новый пресет (= отдельная игра); в главном меню игры его можно выбрать.
