...
impl INode3D for Game3D {
    fn init(base: Base<Node3D>) -> Self {
        Self {
            ...
            minimap_bg: None, minimap_rect: None, minimap_dot: None, minimap_floor: Vec::new(),
...
impl Game3D {
    fn build_environment(&mut self) {
        ...
        env.set_ambient_light_color(Color::from_rgba(0.32, 0.22, 0.38, 1.0));
        env.set_ambient_light_energy(1.15);
        env.set_fog_enabled(true);
        env.set_fog_light_color(Color::from_rgba(0.10, 0.05, 0.14, 1.0));
        env.set_fog_density(0.010);

        // bloom/glow — неоновая эстетика
