...
> **Автоматизация:** ручной цикл «промпт → ChatGPT → сохранить → process_sprites.py»
> теперь заменяет `tools/aigen.py` — он сам ходит на сервер генерации (A1111/ComfyUI,
> настройки в `tools/aigen.json`), прогоняет постобработку и кладёт файлы куда нужно.
> Из редактора Godot — кнопка «🎨 ИИ-текстуры» (см. [docs/EDITOR.md](../docs/EDITOR.md)).
> Этот гайд остаётся источником форматов и рецептов промптов.
